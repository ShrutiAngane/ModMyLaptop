...
const Navbar = () => {
  ...
  return (
    <div>
        <nav className={`flex flex-row-reverse fixed md:flex md:flex-row w-[100%] pl-[1.5em] pr-[1.5em] pt-3 pb-3 justify-between items-center bg-[#15273C] z-10`}>
            <div>
                <NavLink to='/'><img src={logo} alt='modmylaptop' className='w-[50px] h-[65px] z-10'></img></NavLink>
            </div>
            ...
            <ul className={`hidden md:flex md:items-center gap-4 lg:gap-8`}>
                {navlinks.map((element)=>{
                    return typeof(element.path)=='string'?<NavLink to={element.path} className={'text-[#FFFFFF] font-semibold md:text-[1rem] lg:text-[1.2rem] linkwhite' }
                    key={element.id}>{element.text}</NavLink>:
                    ...
                    <ul className='flex flex-col items-start gap-10'>
                        {navlinks.map((element)=>{
                            return <div className='flex items-center gap-3' key={element.id}>
                                    ...
                                    {typeof(element.path)=='string'?<NavLink to={element.path} className='text-[#15273C] font-semibold text-[1rem] xs:text-[1.2rem] linkblue' onClick={handleToggle}>{element.text}</NavLink>:
